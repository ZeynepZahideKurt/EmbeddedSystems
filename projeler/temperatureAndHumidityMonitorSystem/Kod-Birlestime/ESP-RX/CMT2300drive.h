...
#define LBD_STATUS_FLAG   (1<<7)    //0x80     LBD_FLG LBD active (successful low voltage detection) interrupt flag.
#define COLLISION_ERR_FLAG  (1<<6)    //0x40   COL_ERR_FLG
#define DC_ERR_FLAG     (1<<5)    //0x20       PKT_ERR_FLG
#define PREAMBLE_PASS_FLAG  (1<<4)    //0x10   PREAM_OK_FLG
#define SYNC_PASS_FLAG    (1<<3)    //0x08     SYNC_OK_FLG
#define NODE_PASS_FLAG    (1<<2)    //0x04     NODE_OK_FLG
#define CRC_PASS_FLAG   (1<<1)    //0x02       CRC_OK_FLG
#define RX_DONE_FLAG    (1<<0)    //0x01       PKT_OK_FLG
...
void cmt2300aEasy_vSoftReset(void)
{
  ...
  delayMicroseconds(1000);//       //enough? ORJ:1
}
...
**Output:   none
**********************************************************/
/* Anten anahtarını etkinleştirirseniz, GPIO1/GPIO2, RX_ACTIVE/TX_ACTIVE çıkışını verir ve GPIO1/GPIO2 aracılığıyla INT1/INT2 çıkışını yapamaz */
...
void cmt2300aEasy_vEnableWrFifo(void) //AYNI
{
  unsigned char tmp;
...
void cmt2300aEasy_vEnableRdFifo(void) //AYNI
{
  unsigned char tmp;
  tmp = spi3Class_bSpi3Read(CMT23_FIFO_CTL);
  tmp &= ~SPI_FIFO_RD_WR_SEL;
...
//  pinMode(GPIO1, INPUT);
  ...
  tmp |= EEP_CPY_DIS;
  tmp &= (~RSTN_IN_EN);    //Disable RstPin //değişmeler başlıyor
  spi3Class_vSpi3Write(((unsigned short int)CMT23_MODE_STA << 8) + tmp); //CMT2219B_WriteReg(CMT2219B_CUS_MODE_STA, tmp);
